...
@Tag(name = "11. Gasless Relayer (가스비 대납 릴레이어)", description = "투자자 가스 대납 및 오프체인 논스 제공 API")
@RestController
...
public class RelayerController {
    ...
    @Operation(summary = "유효 논스(Nonce) 조회", description = "특정 지갑 주소의 중복 호출 방지용 다음 논스 값을 조회합니다.")
    ...
    @Operation(summary = "가스비 대납 자산 이체", description = "서명 정보를 검증한 후, 가스비 없이 자산을 다른 주소로 안전하게 대납 이체합니다.")
